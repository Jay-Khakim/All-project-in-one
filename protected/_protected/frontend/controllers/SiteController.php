...
//------------------------------------------------------------------------------------------------//
    ...
    {
        $local = Company::find()->where(['type' => Company::TYPE_LOCAL])->limit(4)->all();
        $foreign = Company::find()->where(['type' => Company::TYPE_FOREIGN])->limit(4)->all();
        $best = Product::find()->where(['status' => true])->orderBy('sort desc')->limit(10)->all();
        $products = Product::find()->where(['status' => true])->orderBy('id desc')->limit(12)->all();
        $blogs = Blog::find()->where(['status' => true])->limit(4)->all();
    ...
    public function actionLoading(){
        ...
        if(\Yii::$app->request->isAjax){
            ...
            if(isset($type) && $name == 'company'){
                $query = Company::find()->where(['status' => true , 'type' => $type]);
                $allCount = $query->count();
                $companies = $query->orderBy('id desc')->offset($count)->limit(8)->all();
                $partial = $this->renderAjax('partials/company' , [
                    'companies' => $companies
                ]);
        ...
        if($type == null && $name == 'products'){
            $query = Product::find()->where(['status' => true]);
                $allCount = $query->count();
                $products = $query->orderBy('id desc')->offset($count)->limit(8)->all();
                $partial = $this->renderAjax('partials/product' , [
                    'products' => $products
                ]);
        ...
        if($type == null && $name == 'blogs'){
            $query = Blog::find()->where(['status' => true]);
                $allCount = $query->count();
                $blogs = $query->orderBy('id desc')->offset($count)->limit(8)->all();
                $partial = $this->renderAjax('partials/blog' , [
                    'blogs' => $blogs
                ]);
        ...
        if($type == null && $name == 'sub_category'){
            $subCategory = SubCategoryTranslate::findOne(['url' => $subCategory]);
            $query = Product::find()->where(['status' => true])->andWhere(['sub_category_id' => $subCategory->sub_category_id]);
                $allCount = $query->count();
                $models = $query->orderBy('id desc')->limit(20)->all();
                $partial = $this->renderAjax('partials/sub-category' , [
                    'models' => $models
                ]);
    ...
    public function actionContact()
    {
        ...
        if($model->load($_POST) && $model->validate()){
            ...
            \Yii::$app->session->setFlash('success' , [
                'data' => "Ma'lumotlar yuborildi. Siz bilan tez orada bog'lanamiz",
                'username' => $model->subject
            ]);
            return $this->refresh();
        }
    ...
    protected function telegram($method , $data=[]){

        define('API_KEY' , '998390236:AAGsT8i1LUinCb6suBulQDj45M0ctXghRK4');
