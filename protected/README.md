...
8. Execute _lang_ controller _init_ action that will populate our lang table with default languages:

   ``` ./yii lang/init ``` or if you are on Windows ``` yii lang/init ```
...
6. Run lang/init again: ``` ./yii lang/init ``` or if you are on Windows ```yii lang/init```

7. Now you can tell your application to use your ```advanced``` database again instead of ```advanced_tests```.
Adjust your ```main-local.php``` config file again.

8. Now you are ready to tell Codeception to use ```advanced_tests``` database.
   
   Inside: ``` _protected/tests/codeception/config/config.php ``` file tell your ```db``` to use 
   ```advanced_tests``` database.

9. Start your php server inside the root of your application: ``` php -S localhost:8080 ``` 
(if the name of your application is advanced, then root is ```advanced``` folder) 

10. To run tests written for frontend side of your application 
   ```cd``` to ```_protected/tests/codeception/frontend``` , run ```codecept build``` and then run your tests.

11. Take similar steps like in step 9 for backend and common tests.
